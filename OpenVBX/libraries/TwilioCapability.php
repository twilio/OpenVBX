...
class TwilioCapability {
   ...
   */
  public function allowClientOutgoing($appSid, array $appParams=array(), $clientName=null) {
    // If clientName is specified, it must be a non-zero length alphanumeric string
    if (isset($clientName)) {
      if (preg_match('/\W/', $clientName)) {
        throw new InvalidArgumentException(
          'Only alphanumeric characters allowed in client name.');
