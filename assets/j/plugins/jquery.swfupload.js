...
(function($){
	...
	// Addition done by Twilio to unload the flash object
	$.fn.swfuploadUnload = function(){
		return this.each(function(index, element){
			var swfu;
			swfu = $(this).data('__swfu');
			
			if (swfu != null) {
				var movieName = swfu.movieName
				var buttonPlaceholder = swfu.settings.button_placeholder;

				var parentDiv = $("#" + movieName).parent();
			
				// Destory will remove the <object> tag
				var result = swfu.destroy();

				// But, it won't put back the place holder element and that needs to be there if we
				// are going to be able to reinstantiate the uploader later
				parentDiv.append(buttonPlaceholder);

				$(this).data('__swfu', null);

				// Make sure none of the old event handlers contine to fire.
				$(this).unbind();
			}
		});
	};
